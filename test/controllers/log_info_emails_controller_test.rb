...
class LogInfoEmailsControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @communications_user = User.create!(firstname: "bob", lastname: "bob", email: "comm_user@example.com", password: "password", password_confirmation: "password", confirmed_at: Time.current, role: roles(:Communications))
    @membership = memberships(:member6)
    @person = people(:bobsmith)
    @person.EmailAddress = "comm_user@example.com"
    @person.save
  end

  test "display an edit form" do
    login @communications_user
